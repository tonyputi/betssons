                    ...
                    <div class="border-r border-b border-l border-gray-400 lg:border-t lg:border-gray-400 bg-white p-4 flex flex-col leading-normal">
                        ...
                        <div class="pt-4 pb-2">
                            {this.state.movie.genres && this.state.movie.genres.map((genre, index) => (
                                <span key={index} class="inline-block bg-gray-200 rounded-full px-3 py-1 text-sm font-semibold text-gray-700 mr-2 mb-2">{ genre }</span>
                            ))}
                        </div>
                    </div>
                    <img class="object-contain w-full border-r border-b border-l" src={"/assets/images/movie-covers/" + this.state.movie.img} />
                </div>
            </div>
        );
    }
